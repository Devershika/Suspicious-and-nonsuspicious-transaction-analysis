...
Original file is located at
    https://colab.research.google.com/drive/1qMJa5DHddhju8dzkANU5T_5uXYzJzxsn
"""
...
for _ in range(num_rows):
    ...
    transaction_type = random.choice(['Credit', 'Debit', 'Cash', 'Cheque'])
    currency = 'INR'
...
"""**Inference** **of** **the** **following** **graph** : The box plot compares the daily transaction limits across different transaction types (labeled 0, 1, 2, and 3) based on whether they were flagged as "Suspicious" or "Non-Suspicious." The median daily limits and interquartile ranges are similar for both categories across all transaction types, indicating minimal variation in daily limits between flagged and non-flagged transactions."""
...
X = df.drop(columns=['Suspicious Flag'])  # Features: all columns except the target
y = df['Suspicious Flag']

#Standardizes the data
scaler = StandardScaler()
# Selects only numerical features for scaling
numerical_features = X.select_dtypes(include=['number'])
X_scaled = scaler.fit_transform(numerical_features)
# Optionally, you can create a new DataFrame with the scaled data
X_scaled_df = pd.DataFrame(X_scaled, columns=numerical_features.columns, index=X.index)
# To include categorical columns back, you'd need to use one-hot encoding or other suitable methods

pca = PCA(n_components=None)
X_pca = pca.fit_transform(X_scaled) # Fit PCA on scaled data and transform it
...
target_column = 'Suspicous Flag'
input_features = df.columns[df.columns != target_column]
output_feature = target_column

print("Input Features (Independent Variables):")
